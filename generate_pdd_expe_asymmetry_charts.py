#!/usr/bin/env python3
# generate_pdd_expe_asymmetry_charts.py
# Generates high-fidelity quantitative asymmetry charts for PDD and EXPE.
...
def generate_chart(prices_df, ticker, brand_color):
    ticker_prices = prices_df[ticker].dropna()
    print(f"\nProcessing {ticker}: {len(ticker_prices)} daily data points.")
    ...
    ax1.plot(ticker_prices.index, ticker_prices.values, color=brand_color, linewidth=2.2, label=f'{ticker} Adj Close Price')
    ...
    ax1.annotate(f"${ticker_prices.iloc[-1]:.2f}", 
                 ...
                 xytext=(ticker_prices.index[-1] - pd.Timedelta(days=80), ticker_prices.iloc[-1] * 0.90),
    ...
    plt.close()
    print(f"[+] Successfully generated and saved {ticker} asymmetry chart -> {artifact_output_path}")

def main():
    print("[1/2] Loading daily price matrix...")
    ...
    # Generate for PDD (Brand Color: Electric Magenta/Coral)
    if 'PDD' in prices_df.columns:
        generate_chart(prices_df, 'PDD', '#ff007f')
    else:
        print("[!] PDD missing from daily_prices.csv")
        
    # Generate for EXPE (Brand Color: Premium Cobalt Blue)
    if 'EXPE' in prices_df.columns:
        generate_chart(prices_df, 'EXPE', '#0077ff')
    else:
        print("[!] EXPE missing from daily_prices.csv")
